...
import { SinglePlayerGameState } from '@/app/hooks/useSinglePlayerGame';

interface SinglePlayerGameScreenProps {
  gameState: SinglePlayerGameState;
  onSubmitAnswer: (answer: number) => { correct: boolean };
}

export const SinglePlayerGameScreen: React.FC<SinglePlayerGameScreenProps> = ({
  gameState,
  onSubmitAnswer,
}) => {
      ...
      if (!isNaN(numericAnswer) && numericAnswer > 0) {
        ...
        const timeoutId = setTimeout(() => {
          if (answer === numericAnswer.toString()) { // Make sure answer hasn't changed
            const result = onSubmitAnswer(numericAnswer);
            setAnswerFeedback(result);
            // Clear feedback quickly
            setTimeout(() => setAnswerFeedback(null), 300);
          }
        }, 100);
      ...
      <div className="text-center">
        <div className="mb-8">
          ...
          <div className="flex items-center justify-center space-x-4 text-6xl font-normal text-black mb-8">
            <span>{gameState.currentProblem?.question || 'Loading...'}</span>
            <span>=</span>
            <input
              ref={inputRef}
              type="text"
